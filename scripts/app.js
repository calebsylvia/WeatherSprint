...
async function success(position) {
  ...
  if (searchInput.value) {
    const searchProm = await fetch(
      `http://api.openweathermap.org/geo/1.0/direct?q=${searchInput.value}&limit=1&appid=${weatherKey}`
    );
  ...
  if (data.snow) {
    ...
    snow.classList.add("fa-umbrella");
    down.innerText = "Precipitation: ";
  }
  ...
  async function forecastCall() {
    ...
    day1.innerText = `${day1Day.toLocaleDateString("default", {
      weekday: "long",
    })}`;
    day2.innerText = `${day2Day.toLocaleDateString("default", {
      weekday: "long",
    })}`;
    day3.innerText = `${day3Day.toLocaleDateString("default", {
      weekday: "long",
    })}`;
    day4.innerText = `${day4Day.toLocaleDateString("default", {
      weekday: "long",
    })}`;
    day5.innerText = `${day5Day.toLocaleDateString("default", {
      weekday: "long",
    })}`;
    ...
    day1HL.innerHTML = `H: ${Math.floor(
      Math.max(...day1max)
    )}\u00B0 <br>L: ${Math.floor(Math.min(...day1min))}\u00B0`;
    day2HL.innerHTML = `H: ${Math.floor(
      Math.max(...day2max)
    )}\u00B0 <br>L: ${Math.floor(Math.min(...day2min))}\u00B0`;
    day3HL.innerHTML = `H: ${Math.floor(
      Math.max(...day3max)
    )}\u00B0 <br>L: ${Math.floor(Math.min(...day3min))}\u00B0`;
    day4HL.innerHTML = `H: ${Math.floor(
      Math.max(...day4max)
    )}\u00B0 <br>L: ${Math.floor(Math.min(...day4min))}\u00B0`;
    day5HL.innerHTML = `H: ${Math.floor(
      Math.max(...day5max)
    )}\u00B0 <br>L: ${Math.floor(Math.min(...day5min))}\u00B0`;
